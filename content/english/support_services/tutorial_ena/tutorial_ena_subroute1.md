...
<div class="collapse" id="collapseExample">
  <div class="card card-body">
        <span>Some metadata fields are mandatory. However, in some cases, data for these fields is unavailable for some reason. This will not prevent submission, but any such missing data must be reported appropriately in ENA. In order to do this, users shoud fill the respective fields using the appropriate INSDC-approved term, which are used to indicate not only both that data is missing, but also why. For more information about reporting missing metadata values and the INSDC-approved terms that can be used, see <a href="https://ena-docs.readthedocs.io/en/latest/submit/samples/missing-values.html?highlight=insdc%20missing#reporting-missing-values">here</a>. </span>
  </div>
</div>
...
To register a sample, locate the Samples menu (coloured green), and click on the 'Register Samples' option. In the subsequent interface, click 'Upload Filled Spreadsheet' and upload the example spreadsheet 'sample_spreadsheet.tsv'. You should then see a pop-up window indicating that a successful submission has been made and that the 3 new sample accession numbers.
...
Manifest files can be created using a text editor of your choice (e.g. Notepad). Open one of the manifest files in the *runs* subfolder of the exmple data (e.g. paired_fastq_manifest_sample1.txt) to see an example of a typical manifest file for SARS-CoV-2 sequence data. Note though, that the field values might differ for your project.

It is important to note that you need to ***create one manifest file per sequence file***. We recommend creating a template manifest file in which all field values that are shared between all the samples are filled in (e.g. STUDY, INSTRUMENT, LIBRARY_SOURCE).

The field values for STUDY and SAMPLE need taken from the metadata objects that you submitted earlier:

* Go to the Webin submission portal, open the Dashboard menu (top left of the submission portal page), and click on **Studies Report**. Copy the accession number (starting with PRJEB) and insert it into the manifest file as the value for the STUDY metadata field.
...
A failed validation will provide direction to a report file where you can find a list of error messages explaining the reason for the failure. You can use this to address any faults before re-attempting the validation step.
...
<!-- LIANE - END OF 03-submit-raw-reads -->
...
* Go back to the Dashboard menu (top left of the Webin Submissions Portal) and click on **Samples Report**. Locate the accession number (starting with ERS) for the sample that the sequence file belongs to. Copy this and paste it into the manifest file as the SAMPLE field value.
...
A failed validation will provide directions to a report file that contains a list of error messages explaining the reason for the failure. You can address any issue(s) before re-attempting the validation.

Following a successful validation, you can do a test submission (done by using the `-submit` option in place of `-validate` in the commands given above). Continue using the test server (by keeping the `-test` option), and keep the same input and output directories as for the validation option.
